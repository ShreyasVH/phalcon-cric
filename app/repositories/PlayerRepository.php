...
class PlayerRepository extends Injectable
{
    ...
    public function get_fielding_stats(FilterRequest $filter_request): StatsResponse
    {
        ...
        $count_query = "select count(distinct p.id) as count from fielder_dismissals fd " .
            "inner join match_player_map mpm on mpm.id = fd.match_player_id " .
            "inner join players p on p.id = mpm.player_id " .
            "inner join matches m on m.id = mpm.match_id " .
            "inner join series s on s.id = m.series_id " .
        ...
        if(empty($sort_list))
        {
            $sort_list[] = $this->get_field_name_for_display("fielderCatches") . " desc";
        }
        $query .= " order by " . implode(", ", $sort_list);
        ...
        foreach($result as $row)
        {
            $stat_list[] = [
                'id' => (string) $row['playerId'],
                'name' => $row['name'],
                'fielderCatches' => (string) $row['fielderCatches'],
                'keeperCatches' => (string) $row['keeperCatches'],
                'stumpings' => (string) $row['stumpings'],
                'runOuts' => (string) $row['runOuts']
            ];
        }
